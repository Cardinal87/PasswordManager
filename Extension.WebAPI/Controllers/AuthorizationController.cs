...
namespace Extension.WebAPI.Controllers
{
    ...
    public class AuthorizationController : Controller
    {
        ...
        private IWritableOptions<AuthorizationOptions> _options;
        public AuthorizationController(DatabaseClient dbclient, IWritableOptions<AuthorizationOptions> options)
        {
            _dbclient = dbclient;
            _options = options;
        }
        ...
        {
            var inst = Models.DataConnectors.DbConnectionStringFactory.GetInstance();
            if (String.IsNullOrEmpty(inst.ConnectionString)) return Unauthorized();
            var uri = new Uri(url);
            var host = uri.Host;
        ...
        [HttpPost("postdata")]
        public async Task<IActionResult> SaveData([FromQuery]string url, [FromQuery] string password, [FromQuery] string login)
        {
            var uri = new Uri(url);
            var host = uri.Host;
        ...
        public IActionResult SetPassword()
        {
            ...
            if (auth != null && auth.StartsWith("Basic"))
            {
                if (AuthenticationHeaderValue.TryParse(auth,out var encodedPassword) && encodedPassword.Parameter != null)
                {
                    ...
                    {
                        DataSource = _options.Value.ConnectionString,
                        Password = key
                    };
